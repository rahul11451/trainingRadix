    ...
    <div class="row mb-3">
        ...
        <div class="col-lg-10 lg:ps-5 pt-4 column10">
            <div class="p-md-5 bg-white border rounded shadow-lg border-primary m-5">
                ...
                <div class="m-lg-5 m-2 screen text-primary">
                    <form action="" method="post">
                        <label for="">Enter number:
                            <input type="text" placeholder="Enter number" class="border border-primary rounded form-control" name="num"  <?php $num=$_POST['num']; echo "value='$num'"; ?>  required>
                            <span class="text-danger"><?php if($_SERVER['REQUEST_METHOD'] == "POST"){ if(empty($_POST['num'])){echo 'enter number';} }?></span>
                        </label>
                        <br>
                        <br>
                        <button class="btn-outline-primary btn">click me to check reverse number</button>
                    </form>
                </div>
                <?php
                ...
                if ($num > 0) {
                    $result = strrev($num);
                    echo '<h4 class="mb-5 text-center text-primary"><span class="text-decoration-underline ">Your entered number is: ' . $num . '<br><br></span> The reverse number is: '.$result.'</h4>';
                } else {
