<?php
session_start();
// echo '<br>Nensi<br>File: '. __FILE__.'<br>Line: '.__LINE__.'<br><pre>';print_r($_POST);echo '</pre>'; die();
echo '<br>Nensi<br>File: '. __FILE__.'<br>Line: '.__LINE__.'<br><pre>';print_r($_SESSION);echo '</pre>'; die();
$paths = '/php/';
$key = '';
$queSet = array(
    'qSet1' => array('question' => 'How did you hear about us?', 'answer' => array('Instagram', 'Facebook', 'word of mouth', 'Podcast', 'other')),
    'qSet2' => array('question' => 'How likely are you to recommend our website to a friend?', 'answer' => array(1, 2, 3, 4, 5)),
    'qSet3' => array('question' => 'Whay are you leaving?', 'answer' => array('I need time to think', 'No worries, i will br back', 'I need more information', 'The item is too expensive', 'I just browsing')),
    'qSet4' => array('question' => 'Need Help?', 'answer' => array('Yes', 'No', 'sorry', 'Ofcourse', 'May be')),
    'qSet5' => array('question' => 'Your Opinion means the world to us?', 'answer' => array('Very satisfied', 'Satisfied', 'Neutral', 'Dissatisfied', 'Very Dissatisfied'))
);
$question_input = '';
        ...
        if (is_string($value)) {
            ...
            foreach ($value as $keys => $option) {
                $question_input .= '<input type="checkbox" ' ?> <?php 
                    ...
                    } ?> <?php $question_input .= ' name="answer[' . $set . ']['.$keys.']" id="" class="form-check-input border-primary" value = "' . $option . '"  >
                
                <label for="" class="form-check-label"> ' . $option . '</label><br>';
            }
        }
        $question_input .= '</li>';
    }
...
if(isset($_SESSION['answer'])){
    ...
    foreach($_SESSION['answer'] as $queKey => $optionArray){
        echo '<br>Nensi<br>File: '. __FILE__.'<br>Line: '.__LINE__.'<br><pre>';print_r($queSet[$queKey]['question']);echo '</pre>'; 
        foreach($optionArray as $ansKey => $ansValue){
            echo '<br>Nensi<br>File: '. __FILE__.'<br>Line: '.__LINE__.'<br><pre>';print_r($queSet[$queKey]['answer'][$ansKey]);echo '</pre>'; 
        }
